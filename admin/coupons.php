        ...
        if (!$errors) {
            ...
            if ($edit_id) {
                $pdo->prepare("UPDATE coupons SET company_id=?, code=?, title_ar=?, title_en=?, description_ar=?, description_en=?,
                               discount_text=?, redirect_url=?, expires_at=?, is_active=? WHERE id=?")->execute([...$data, $edit_id]);
            } else {
                $pdo->prepare("INSERT INTO coupons (company_id, code, title_ar, title_en, description_ar, description_en,
                               discount_text, redirect_url, expires_at, is_active) VALUES (?,?,?,?,?,?,?,?,?,?)")->execute($data);
            }
...
<?php if ($action === 'add' || $action === 'edit'): ?>
  <div class="panel">
    ...
    <form method="post">
      ...
      <div class="field">
        <label>رابط التحويل الخاص بهذا الكوبون</label>
        <input type="text" name="redirect_url" value="<?= e($editing['redirect_url'] ?? '') ?>" placeholder="https://example.com/special-offer?ref=...">
        <small>عند نسخ العميل لكود الكوبون في الموقع، سيتم تحويله مباشرة لهذا الرابط. اتركه فارغًا لاستخدام رابط الأفلييت الأساسي للشركة بدلاً منه.</small>
      </div>
...
<?php else: ?>
  ...
  <table class="admin-table">
    <thead><tr><th>الكود</th><th>الشركة</th><th>العنوان</th><th>الخصم</th><th>رابط التحويل</th><th>نشط</th><th></th></tr></thead>
    <tbody>
        ...
        <tr>
          ...
          <td>
            <?php if (!empty($cp['redirect_url'])): ?>
              <span class="badge" title="<?= e($cp['redirect_url']) ?>">رابط خاص</span>
            <?php else: ?>
              <span style="color:var(--muted);font-size:13px">رابط الشركة</span>
            <?php endif; ?>
          </td>
          <td><?= $cp['is_active'] ? '✔' : '—' ?></td>
